// এই ইভেন্টটি নিশ্চিত করে যে, সম্পূর্ণ HTML লোড হওয়ার পরেই জাভাস্ক্রিপ্ট কোডটি রান হবে।
document.addEventListener('DOMContentLoaded', function() {
    
    // --- ডেটা এবং ডিফল্ট সেটিংস ---
    // সকল পরীক্ষার তথ্য এখানে একটি অবজেক্টে রাখা হয়েছে।
    ...
    // --- গ্লোবাল সেটিংস ভেরিয়েবল ---
    let settings;
    // --- সকল ডিফল্ট সেটিংস এখানে রাখা হয়েছে ---
    ...
    /**
     * ব্রাউজারের লোকাল স্টোরেজ থেকে সেভ করা সেটিংস লোড করে। 
     * যদি কোনো সেটিং সেভ করা না থাকে, তাহলে ডিফল্ট সেটিংস ব্যবহার করে।
     */
    ...
    /**
     * বর্তমান সেটিংস অনুযায়ী পুরো ইন্টারফেস আপডেট করে।
     * যেমন: থিম, ভাষা, ফন্ট, রঙ ইত্যাদি প্রয়োগ করে।
     */
    function applySettings() {
        // থিম (লাইট/ডার্ক) প্রয়োগ
        ...
        // সেটিংস প্যানেলের অন্যান্য কন্ট্রোল আপডেট
        ...
        // সবশেষে টেবিলটি নতুন করে তৈরি করা হয়
        populateTable();
    }

    /**
     * নির্বাচিত বিভাগ অনুযায়ী পরীক্ষার তালিকা ফিল্টার করে।
     */
    ...
    /**
     * ফিল্টার করা পরীক্ষার তালিকা দিয়ে টেবিলটি তৈরি করে।
     */
    ...
    // --- সকল ইভেন্ট লিসেনার ---
    // সেটিংস প্যানেল খোলা এবং বন্ধ করার জন্য
    ...
    // বিভাগ, ভাষা, এবং থিম পরিবর্তনের জন্য
    ...
    // ফন্ট এবং লেখার আকার পরিবর্তনের জন্য
    document.getElementById('font-bn-select').addEventListener('change', (e) => {localStorage.setItem('appFontBn', e.target.value); loadSettings(); applySettings();});
    document.getElementById('font-en-select').addEventListener('change', (e) => {localStorage.setItem('appFontEn', e.target.value); loadSettings(); applySettings();});
    ...
    document.querySelectorAll('.color-input-group input[type="color"]').forEach(picker => {
        picker.addEventListener('input', (e) => {
            ...
            root.style.setProperty(cssVar, e.target.value); // তাৎক্ষণিক পরিবর্তন
            localStorage.setItem('appColors', JSON.stringify(settings.colors)); // সম্পূর্ণ অবজেক্ট সেভ
        });
    });

    // সকল সেটিংস রিসেট করার জন্য
    ...
    function renderCalendar() {
         ...
         const langDict = translations[settings.lang];
         monthYearEl.textContent = `${langDict.months[calDate.getMonth()]} ${new Intl.DateTimeFormat(settings.lang === 'bn' ? 'bn-BD' : 'en-GB', { year: 'numeric' }).format(calDate)}`;
         ...
         for (let i = 1; i <= lastDate; i++) {
             ...
             dayDiv.textContent = new Intl.DateTimeFormat(settings.lang === 'bn' ? 'bn-BD' : 'en-GB', { day: 'numeric' }).format(new Date(calDate.getFullYear(), calDate.getMonth(), i));
             if (i === new Date().getDate() && calDate.getMonth() === new Date().getMonth() && calDate.getFullYear() === new Date().getFullYear()) {
    ...
    // --- লাইভ টাইমার (প্রতি সেকেন্ডে আপডেট) ---
    setInterval(() => {
        const now = new Date();
        const langDict = translations[settings.lang];
        ...
        // লাইভ ঘড়ি আপডেট
        if (settings.lang === 'bn') {
            let hours = now.getHours(); let periodKey;
            ...
            const timeString = new Intl.DateTimeFormat('bn-BD', {hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: false}).format(now);
            document.getElementById('live-clock').textContent = `${langDict.time_periods[periodKey]} ${timeString}`;
        } else {
        ...
        // পরীক্ষার কাউন্টডাউন টাইমার আপডেট
